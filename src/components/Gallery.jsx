...
const DEFAULT_IMAGES = Object.keys(imageModules)
  ...
  .map(path => ({
    src: `/ufuq-landing-page/gallery/${path.split('/').pop()}`,
    alt: 'Teens Meet 2026'
  }));
...
const Gallery = () => {
  ...
  const onTileClick = (el, src) => {
    if (stateRef.current.isDragging || stateRef.current.hasMoved || (performance.now() - stateRef.current.lastDragEndAt < 80) || stateRef.current.opening) return;
    
    stateRef.current.opening = true;
    document.body.classList.add('dg-scroll-lock');
    const parent = el.parentElement;
    stateRef.current.focusedEl = el;
    ...
    stateRef.current.originalTilePos = { left: tileR.left, top: tileR.top, width: tileR.width, height: tileR.height };
    ...
    overlay.style.transition = `transform ${DEFAULTS.enlargeTransitionMs}ms ease, opacity ${DEFAULTS.enlargeTransitionMs}ms ease`;
  ...
  useEffect(() => {
    ...
    const onPointerDown = (e) => {
      if (stateRef.current.focusedEl) return;
      stopInertia();
    ...
    const onPointerMove = (e) => {
      if (!stateRef.current.isDragging || !stateRef.current.startPos) return;
      const dx = e.clientX - stateRef.current.startPos.x;
      const dy = e.clientY - stateRef.current.startPos.y;
      if (!stateRef.current.hasMoved && (dx * dx + dy * dy > 16)) stateRef.current.hasMoved = true;
    ...
    const onPointerUp = (e) => {
      ...
      const wasDrag = stateRef.current.hasMoved;
      if (stateRef.current.hasMoved) stateRef.current.lastDragEndAt = performance.now();
      stateRef.current.hasMoved = false;

      let vx = clamp(stateRef.current.instVx * 15, -1.2, 1.2);
      let vy = clamp(stateRef.current.instVy * 15, -1.2, 1.2);
    ...
    const onScrimClick = () => {
      if (!stateRef.current.focusedEl) return;
      const parent = stateRef.current.focusedEl.parentElement;
      ...
      animEl.style.cssText = `position:absolute; left:${overlayRelative.left}px; top:${overlayRelative.top}px; width:${overlayRelative.width}px; height:${overlayRelative.height}px; z-index:9999; border-radius:30px; overflow:hidden; box-shadow:0 10px 30px rgba(0,0,0,.35); transition:all ${DEFAULTS.enlargeTransitionMs}ms ease-out; pointer-events:none;`;
      ...
      const rootLeft = stateRef.current.originalTilePos.left - rootRect.left;
      const rootTop = stateRef.current.originalTilePos.top - rootRect.top;

      requestAnimationFrame(() => {
        animEl.style.left = rootLeft + 'px';
        animEl.style.top = rootTop + 'px';
        animEl.style.width = stateRef.current.originalTilePos.width + 'px';
        animEl.style.height = stateRef.current.originalTilePos.height + 'px';
      ...
      animEl.addEventListener('transitionend', () => {
        ...
        stateRef.current.focusedEl.style.visibility = '';
        parent.removeAttribute('data-focused');
        stateRef.current.focusedEl = null;
      ...
      >
        <main ref={mainRef} className="sphere-main">
          ...
          <div ref={viewerRef} className="viewer">
            <div ref={scrimRef} className="scrim"></div>
